...
\item \textbf{模型量化与压缩}
\begin{itemize}
  \item \textbf{Qwen-VL}: 采用 AWQ、SmoothQuant、GPTQ、SpinQuant 量化至 FP4（权重+激活），精度优于 W4A16 baseline，同时 Prefill 延迟降低 50\%。集成 Eagle 投机解码，Decode 阶段加速 $\sim$4.7$\times$。
  \item \textbf{Orion}（VLA 模型）: 组合 Spin + Scale + SVD 分解 + 混合精度，保持 FP4 模型精度，延迟较 FP8 baseline 降低 $>$15\%。
  \item \textbf{RDQuant} -- 率失真最优混合精度量化：提出基于率失真优化的逐输出通道混合精度框架，通过 Lagrangian 松弛在给定比特预算下最优分配比特宽度（NVFP4/FP8/FP16）。设计混合精度 GEMM 执行 pipeline，包括精度感知通道重组、权重重排及多格式 kernel 分发。在 Qwen3-4B 上以 $\sim$5.3 bits/weight 超越 BF16 baseline 困惑度。已集成至 vLLM，在 NVIDIA RTX 5090 上实现 1.9$\times$ 加速。
  \item \textbf{Entropy-Aware NVFP4 量化}：提出逐 block FP8 scale 优化方法，联合最小化重建误差与码字熵，提升后端压缩 IP 的可压缩性。结合负零消除，在精度无损的前提下实现 $\sim$12\% 压缩率提升，输出格式兼容标准 NVFP4。
...
\begin{itemize}
  ...
  \item \textit{大模型算子}:
  \begin{itemize}
    \item \textbf{W4A16 推理优化}: 针对 Decode 阶段特定 shape（尤其 $M\!=\!1$）优化 W4A16 GEMV kernel，带宽利用率达 $\sim$80\%。在 Marlin 后端实现 Fused SwiGLU-FFN kernel，重排 gate/up 权重实现 register 级 GeLU + Mul 融合。
    \item 将 \textbf{MoE W4A16} kernel 集成至内部 TensorRT-LLM pipeline，包括离线权重重排与图集成，支持 Qwen3-MoE 端到端部署。
    \item \textbf{FP8 GEMM} 优化: 在 CUTLASS 中添加 M 维度对齐至 48 整数倍的支持，启用 PTX 层 1.5$\times$ 吞吐特性。修复 TMA store partial-write 导致的 2$\times$ 写流量问题。
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  ...
  \item 针对 VAD-Base 中 Deformable Attention 进行瓶颈分析，导出 \textbf{double ALU} 硬件需求，定量评估性能收益并交付 GPGPU 团队。
...
\begin{itemize}
  \item 通过混合精度、FlashAttention、任务流水线和内存高效 FFN tiling 优化 LLM 推理。
  \item 开发自动层融合算法和 double buffering 调度。
...
\item \textbf{MLPerf 模型剪枝}: 交付 MLPerf 推理基准的超压缩模型（Resnet-50、Mobilenet、SSD-Resnet34、Bert）。Resnet-50 在 1\% 精度损失内实现 84\% 权重稀疏率和 65\% 激活稀疏率，加速比 $>$2$\times$。
