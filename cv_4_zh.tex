...
\begin{itemize}
  ...
  \item \textbf{KVFloat13 -- vLLM KV Cache 压缩}：设计自定义 13-bit 浮点格式（1S+5E+7M）用于 KV cache 存储，完成 vLLM serving 栈端到端集成——扩展 \texttt{kv\_cache\_dtype} 链路与 packed-byte 显存核算，接入 FlashAttention backend，支持 CUDA Graph capture，开发 cache update 与 decode 热路径的自定义 CUDA 算子。在 Qwen3-4B 端到端 serving 上实现 $\sim$23\% KV cache 容量提升，通过 Nsight Systems + NVTX profiling 验证。
