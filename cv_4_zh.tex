...
\begin{itemize}
  ...
  \item \textbf{RDQuant} -- 率失真最优混合精度量化：提出基于率失真优化的逐输出通道混合精度框架，通过 Lagrangian 松弛在给定比特预算下最优分配比特宽度（NVFP4/FP8/FP16）。在 Qwen3-4B 上以 $\sim$5.3 bits/weight 超越 BF16 baseline perplexity。
  \item \textbf{Entropy-Aware NVFP4 量化}：提出逐 block FP8 scale 优化方法，联合最小化 MSE 与 codeword entropy，面向后端硬件 compression IP。NVFP4 精度无损下实现 $\sim$12\% 压缩率提升，输出格式兼容标准 NVFP4。
  \item 对 Microscaling FP4 和 NVFP4 进行软硬件联合评估，指导量化硬件路线图。
\end{itemize}

\item \textbf{Runtime-level: vLLM Serving 栈集成}
\begin{itemize}
  \item \textbf{RDQuant 集成 vLLM}: 将 mixed-precision GEMM 集成至 vLLM serving 栈，完成 offline channel regrouping、weight repacking pipeline 与 multi-format kernel dispatch。在 NVIDIA RTX 5090 上实现端到端 FP16 \textbf{1.9$\times$} 加速。
  \item \textbf{KVFloat13 vLLM KV Cache 压缩}：设计自定义 13-bit 浮点格式（1S+5E+7M），完成 vLLM serving 栈端到端集成——扩展 \texttt{kv\_cache\_dtype} 链路与 packed-byte 显存核算，接入 FlashAttention backend，支持 CUDA Graph capture，开发 cache update 与 decode 热路径的自定义 CUDA 算子。在 Qwen3-4B 端到端 serving 上实现 $\sim$23\% KV cache 容量提升。
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{RDQuant Mixed-Precision GEMM}: 实现支持逐通道 bit-width dispatch 与 fused dequantization 的 mixed-precision (NVFP4/FP8/FP16) GEMM kernel，支撑 RDQuant 率失真最优比特分配在推理端的落地。
  \item \textbf{W4A16 GEMV}: 针对 Decode 阶段特定 shape（尤其 $M\!=\!1$）优化 kernel，bandwidth utilization 达 $\sim$80\%。
  \item \textbf{Fused SwiGLU-FFN on Marlin}: 重排 gate/up 权重，实现 register 级 GeLU + Mul 融合，消除中间显存读写。
  \item \textbf{FP8 GEMM (CUTLASS)}: 在 CUTLASS 中添加 M 维度对齐至 48 整数倍的支持，启用 PTX 层 1.5$\times$ throughput 特性。修复 TMA store partial-write 导致的 2$\times$ write traffic 问题。
\end{itemize}

\item \textbf{Hardware-level: Profiling 与软硬件协同设计}
\begin{itemize}
  ...
  \item 针对 VAD-Base 中 Deformable Attention 进行 bottleneck 分析，导出 \textbf{double ALU} 硬件需求，定量评估性能收益并交付 GPGPU 团队。
