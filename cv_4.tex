...
{8+ year work experience, focus on inference optimization on NPU/GPU including model compression/CUDA kernel optimization/compilation optimization/performance modeling...}
...
\begin{itemize}
  ...
  Standard NVFP4 produces near-uniform codeword distributions ($\sim$0\% compressible by backend compression IP). Proposed a per-block FP8 scale optimization method that jointly minimizes reconstruction MSE and block-level Shannon entropy via Lagrangian relaxation ($\text{Score} = \text{MSE} + \lambda \cdot H_b$). Combined with negative-zero elimination ($\sim$3\% extra gain). Achieved $\sim$12\% compression ratio with no accuracy degradation vs.\ standard NVFP4. Output remains standard NVFP4 format --- no inference engine modification required.
...
Implementing state of the art model quantization techniques, including low-bit quantization methods like PACT and SAWB,  Mixed precision methods including HAQ and HAWQ, Nonlinear quantization methods like log-based, outlier-aware and Squantizer. 
...
[9] Yuanyuan Li, \textbf{Xu Qian}, Deepak Mathaikutty, Arnab Raha, Haiyun Hong, "A method to Reduce Workload Overhead for LLM Execution on resource constrained DNN Accelerators", Patent Filed, In Progress 
\end{rSection}
