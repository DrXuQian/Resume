...
Led low-bit quantization research and deployment for VLM/VLA/LLM workloads, covering algorithm design, accuracy optimization, and end-to-end inference acceleration.
\begin{itemize}
  ...
  \item \textbf{Entropy-Aware NVFP4 Quantization}: Developed an entropy-aware scaling strategy that adapts per-block quantization parameters based on weight distribution entropy, improving reconstruction quality for high-entropy (outlier-rich) channels.
...
Developed and optimized CUDA kernels on both custom GPGPU and NVIDIA GPU platforms to accelerate model inference.
\begin{itemize}
  ...
  \item Implemented \textbf{MoE W4A16} kernel in \textbf{TensorRT-LLM} to support mixture-of-experts models with INT4 weight-only quantization. Enabled deployment of \textbf{Qwen3-MoE} on NVIDIA GPUs.
\end{itemize}
