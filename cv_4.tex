...
\begin{itemize}
  \item Optimized performance-critical ops including \textbf{DeformableAttention}, \textbf{GridSample}, \textbf{Voxelization}, \textbf{BevPoolv2}, \textbf{CoordTransformMatrix}, etc. Reduced latency to $\sim$1/6 of MMCV baseline.
  \item Optimized \textbf{W4A16 GEMV} kernels for decode-stage shapes (especially $M\!=\!1$), achieving $\sim$80\% memory bandwidth utilization on custom GPGPU.
  \item Identified and fixed a traffic anomaly in the \textbf{FP8 GEMM} operator on the performance model, and completed kernel modification.
  \item Implemented \textbf{MoE W4A16} kernel on custom GPGPU to support mixture-of-experts models with INT4 weight-only quantization. Enabled deployment of \textbf{Qwen3-MoE}.
\end{itemize}
