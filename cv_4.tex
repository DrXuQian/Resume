...
\item \textbf{End-to-End Model Deployment}:
Led the quantization and deployment of multiple end-to-end models on custom GPGPU architecture.
\begin{itemize}
...
\begin{itemize}
  ...
  \item Conducted bottleneck analysis on latency-dominant operators (especially \textbf{Deformable Attention}) in \textbf{VAD-Base}. Derived the \textbf{double ALU} hardware requirement with quantitative performance gain estimation, and delivered to PPU team as edge SoC design input.
  \item Analyzed hardware optimization options for \textbf{Flash Attention} during LLM \textbf{Prefill} stage. Quantitatively evaluated the performance benefit of \textbf{double SFU}, and delivered to PPU team as edge SoC design input.
...
Performed architecture analysis, deployment, and cross-platform performance benchmarking of latest-generation VLMs across \textbf{NVIDIA Thor} and custom \textbf{PPU}-based edge SoC.
\begin{itemize}
  ...
  \item Deployed and benchmarked the above models on both \textbf{Thor} (GPU) and \textbf{PPU} (custom accelerator) platforms. Conducted end-to-end latency profiling and operator-level bottleneck analysis across platforms.
  \item Identified key performance bottlenecks in Prefill and Decode stages, comparing compute/memory utilization differences between Thor and PPU to guide platform selection and hardware co-optimization.
\end{itemize}
